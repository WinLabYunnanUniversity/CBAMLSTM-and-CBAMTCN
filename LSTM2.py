...
for sample_length in [128]:
    for mod_name in ['signal']:
        ...
        best_model_path = 'result/models/LSTM1/' + str(sample_length) + '/' + str(mod_name) + 'best.h5'
        checkpointer = ModelCheckpoint(best_model_path, verbose=1, save_best_only=True)
        TB_dir = 'result/TB/LSTM1/' + str(mod_name) + '_' + str(sample_length)
        ...
        if model.stop_training:
            # save results
            model.save('result/models/LSTM1/' + str(sample_length) + '/' + str(mod_name) + 'final.h5')
            print('Second stage finished, get the final model')
            save_path = 'C:/Users/admin/Desktop/TCN-with-attention-master/data/result/xls/LSTM1/128/' + str(
                mod_name) + 'Pds.csv'
